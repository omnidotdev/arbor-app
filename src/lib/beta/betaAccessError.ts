 ...
 * ClientError with the raw response attached, so we inspect the per-error
 * extension codes (with a serialized-message fallback), mirroring how auth
 * errors are detected in DefaultCatchBoundary.
 *
...
export const isBetaAccessError = (error: unknown): boolean => {
  ...
  if (response?.errors?.some((entry) => entry.extensions?.code === BETA_CODE)) {
    return true;
  }
